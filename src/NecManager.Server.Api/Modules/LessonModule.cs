...
public sealed class LessonModule : IModule
{
    public IEndpointRouteBuilder MapEndpoints(IEndpointRouteBuilder endpoints)
    {
        _ = endpoints.MapGet("/lessons",
             [Authorize(Policy = PolicyName.IsAdmin)] async (ApiRequestHeaders requestHeaders, [FromServices] ILessonBusiness lessonService, /*BindAsync*/ LessonQueryInput query)
        ...
        _ = endpoints.MapGet("/lessons/{lessonId:int}",
             [Authorize(Policy = PolicyName.IsAdmin)] async (ApiRequestHeaders requestHeaders, [FromServices] ILessonBusiness lessonService, [FromRoute] int lessonId)
        ...
        _ = endpoints.MapPost("/lessons",
             [Authorize(Policy = PolicyName.IsAdmin)] async (ApiRequestHeaders requestHeaders, [FromServices] ILessonBusiness lessonService, [FromBody] LessonCreationInput input)
        ...
        _ = endpoints.MapPut("/lessons/{lessonId:int}",
             [Authorize(Policy = PolicyName.IsAdmin)] async (ApiRequestHeaders requestHeaders, [FromServices] ILessonBusiness lessonService, [FromRoute] int lessonId, [FromBody] LessonUpdateInput input)
